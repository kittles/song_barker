...
def classify_audio (fp):
    wav_data, sr = sf.read(fp, dtype=np.int16)
    ...
    return scores, spectrogram, waveform


def plot_result (scores, spectrogram, waveform):
...
print(classify_audio('sample_dog_barks/008.wav'))


def classify_sample_audio ():
    for fp in glob.glob('sample_dog_barks/*.wav'):
        wav_data, sr = sf.read(fp, dtype=np.int16)
        waveform = wav_data / 32768.0
        # Sampling rate should be 16000 Hz.
        assert sr == 16000
        ...
        plt.savefig(fp.replace('.wav', '.png'))
